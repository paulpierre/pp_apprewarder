...
function smarty_modifier_relative_date($timestamp, $days = false, $format = "M j, Y") {
  ...
  if ($difference < (60*60)) { 
    return intval($difference / 60) . "m ago"; 
  }
  if ($difference < (60*60*2)) { 
    return "1 hour ago"; 
  }
  if ($difference < (60*60*24)) {    
    return intval($difference / (60*60)) . "h ago"; 
  }
  if ($difference < (60*60*24*2)) { 
    return "1 day ago"; 
  }
  if ($difference < (60*60*24*7)) { 
    return intval($difference / (60*60*24)) . "d ago"; 
  }
  if ($difference < (60*60*24*7*2)) { 
    return "1 week ago"; 
  }
  if ($difference < (60*60*24*7*(52/12))) { 
    return intval($difference / (60*60*24*7)) . "w ago"; 
  }
  if ($difference < (60*60*24*7*(52/12)*2)) { 
    return "1 month ago"; 
  }
  if ($difference < (60*60*24*364)) { 
    return intval($difference / (60*60*24*7*(52/12))) . "mo ago"; 
  }
